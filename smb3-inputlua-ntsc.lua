...
function drawStats(x, y)
	...
	--
	if(memory.readbyteunsigned(0x3dd) > 1) then
	drawText(x, y + 63, letterTable("next p " .. math.floor(memory.readbyte(0x515)/8) .. math.fmod(memory.readbyte(0x515), 8)))
	end
	...
	drawText(x, y + 70, letterTable("p kill " .. memory.readbyteunsigned(0x56e) * 2 + pkillsub - 1))
	pkillsub = 1 - pkillsub
	end
	--
	...
	drawText(x, y +112, letterTable(timeCount()))
	--
	for i=0, 4, 1 do
		if(memory.readbyte(0x671+i) == 14 or memory.readbyte(0x671+i) == 24 or memory.readbyte(0x671+i) == 75 or memory.readbyte(0x671+i) == 76) then
